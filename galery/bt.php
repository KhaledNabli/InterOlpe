...
function createThumb($thisImagePath, $thisThumbnailPath)
{
	...
	//$file = $imagePath.'/'.$albumName.'/'.$fileName;
	$file = $thisImagePath;
	//$fileDest = $thumbnailPath.'/'.$albumName.'/'.$fileName.'thumb';
	$fileDest = $thisThumbnailPath;
	//$fileDest = $thumbnailPath.'/'.$albumName.'/'.$fileName;
	
	//print $imagePath;
	//print $file . " to " . $fileDest . '<BR>';
	//print $fileDest;
	...
	if ($gdVersion < 2) {
		...
		if (@imagecopyresized($imageDest, $imageSrc, 0, 0, $deltaX, $deltaY, $outputX, $outputY, $portionX, $portionY)) {
			...
			return "<font face='arial, helvetica' size='small' color='#006633'>CREATED";
	...
	return "<font face='arial, helvetica' size='small' color='#FF0000'>ERROR";
}
